...
lm_data=data.frame(RFNB=i,IC_CPI=i,avg_neRFNB=i,ne_dist=i,nc_dist=i,population=i,CPI=i)
for(y in 1:6){
  for(i in 1:dim(ID_RFNB)[1]){
    index=((y-1)*dim(ID_RFNB)[1])+i
    lm_data$RFNB[index]=ID_RFNB[i,44+y]
    ...
    lm_data$CPI[index]=CPI$Value[CPI$Ref_Date==(2004+y)&CPI$GEO=="Canada"]
  }
}

fit <- lm(RFNB~avg_neRFNB+ne_dist+nc_dist+population+CPI+IC_CPI, data=lm_data)
...
for(y in 1:8){
  for(i in 1:dim(inuit_data)[1]){
    inuit_data[i,54-y]=as.numeric(fit$coefficients[1]
                                  +fit$coefficients[2]*lm_data$avg_neRFNB[i]
                                  +fit$coefficients[3]*lm_data$ne_dist[i]
                                  +fit$coefficients[4]*lm_data$nc_dist[i]
                                  +fit$coefficients[5]*lm_data$population[i]
                                  +fit$coefficients[6]*lm_data$CPI[i]
                                  +fit$coefficients[7]*lm_data$IC_CPI[i]
    )
  }
}
