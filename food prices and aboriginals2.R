...
###### correct for avg_neRFNB and IC_CPI ############
...
for(y in 1:6){
  for(i in 1:dim(ID_RFNB)[1]){
    ID_RFNB[i,63-y]=(ID_RFNB[i,44+y]-ID_RFNB$avg_neRFNB[i])#/InterCity$Value[InterCity$Ref_Date==(2004+y)&InterCity$GEO==(ID_RFNB$nearcity[i])]*100
...
fits=data.frame(b=rep(NA,dim(ID_RFNB)[1]),m=rep(NA,dim(ID_RFNB)[1]),m2=rep(NA,dim(ID_RFNB)[1]))
for(i in 1:dim(ID_RFNB)[1]){
  x=CPI$Value[CPI$Ref_Date>=2005&CPI$Ref_Date<=2010&CPI$GEO=="Canada"]
  x2=InterCity$Value[InterCity$Ref_Date>=2005&InterCity$Ref_Date<=2010&InterCity$GEO==(ID_RFNB$nearcity[i])]
  y=as.numeric(ID_RFNB2[i,45:50])
  if(sum(is.na(y))<4){
    lm_data=data.frame(cbind(y,x,x2))
    fit=lm(lm_data$y~lm_data$x+lm_data$x2,data=lm_data)
    fits[i,]=c(fit$coefficients[1],fit$coefficients[2],fit$coefficients[3])
  }
}

b=mean(fits$b,na.rm=T)
m=mean(fits$m,na.rm=T)
m2=mean(fits$m2,na.rm=T)
...
for(y in 1:8){
  for(i in 1:dim(inuit_data)[1]){
    ...
    index2=InterCity$Ref_Date==(2005)|InterCity$Ref_Date==(2006)|InterCity$Ref_Date==(2007)|InterCity$Ref_Date==(2008)|InterCity$Ref_Date==(2009)|InterCity$Ref_Date==(2010)
    inuit_data[i,54-y]=as.numeric(inuit_data$avg_neRFNB[i] #fit$coefficients[1]
      ...
      +m*(mean(CPI$Value[index])-CPI$Value[CPI$GEO=="Canada"&CPI$Ref_Date==(2004+y)])
      +m2*(mean(InterCity$Value[index2])-InterCity$Value[InterCity$Ref_Date==(2004+y)&InterCity$GEO==(inuit_data$nearcity[i])])
    )
  }
}
