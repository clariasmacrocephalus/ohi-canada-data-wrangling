...
aboriginal_data=data.frame(cbind(name=c(as.character(inuit_data$NAME),as.character(FN_data$BAND_NAME)),
                            type=c(rep("Inuit",length(inuit_data$NAME)),rep("First Nations",length(FN_data$BAND_NAME)))))
aboriginal_data$Lat_Y=as.numeric(c(inuit_data$Lat_Y,FN_data$Lat_Y))
aboriginal_data$Long_X=as.numeric(c(inuit_data$Long_X,FN_data$Long_X))
...
cover=data.frame(matrix(NA,nrow=length(filelist),ncol=length(aboriginal_data$Long_X)))
...
for(t in 1:length(filelist)){
  ...
  for(i in 1:length(aboriginal_data$Long_X)){
    c1 = circle(cbind(aboriginal_data$Long_X[i], aboriginal_data$Lat_Y[i]),300000)
...
#write.csv(cover,"~/Documents/R/OHI/cover.csv")
cover=read.csv("~/Documents/R/OHI/cover.csv")


### calculate mean trend
cover$mean=cover$month
for(i in 1:length(filelist)){
  cover$mean[i]=mean(as.numeric(cover[i,1:length(aboriginal_data$Long_X)]),na.rm=T)
}
...
ice_index_byyear=data.frame(matrix(NA,nrow=length(years),ncol=length(aboriginal_data$Long_X)))
ice_layer=data.frame(matrix(NA,nrow=length(aboriginal_data$Long_X)*length(years),ncol=4))
names(ice_layer)=c("Name","Type","Year","Cover")
for(j in 1:length(aboriginal_data$Long_X)){
  for(i in 1:length(years)){
    index=as.numeric(cover$year)==years[i]
    ice_index_byyear[i,j]=mean(cover[index,j],na.rm=T)
    index2=(j-1)*length(years)+i
    ice_layer$Cover[index2]=mean(cover[index,j],na.rm=T)
    ice_layer$Name[index2]=as.character(aboriginal_data$name[j])
    ice_layer$Year[index2]=years[i]
    ice_layer$Type[index2]=c(rep("Inuit",length(inuit_data$NAME)),rep("First Nations",length(FN_data$BAND_NAME)))[j]
  }
}
## cover in m^2 to cover in %
ice_layer$pCover=0
for(i in 1:length(unique(ice_layer$Name))){
  ice_layer$pCover[ice_layer$Name==unique(ice_layer$Name)[i]]=ice_layer$Cover[ice_layer$Name==unique(ice_layer$Name)[i]]/max(ice_layer$Cover[ice_layer$Name==unique(ice_layer$Name)[i]])
}
write.csv(ice_layer,"~/Documents/R/OHI/ice_layer.csv")
...
plot(years,pcover$mean*100,"l",xlab="Years",ylab="Percent Ice Cover (%)")

jpeg(paste('~/Documents/R/OHI/Figures/ice.jpg',sep=""),height=1200,width=1800,res=400,qual=100)
plot(years,pcover$mean*100,xlab="Year",ylab="Sea-Ice Cover (%)","l")
dev.off()
...
years=unique(cover$year)[2:length(unique(cover$year))]
range=data.frame(matrix(NA,nrow=12,ncol=length(aboriginal_data$Long_X)))
variance=data.frame(matrix(NA,nrow=12,ncol=length(aboriginal_data$Long_X)))


for(m in 1:12){
  cover_m=cover[as.numeric(cover$month)==m,]
  ice_index_bymonth=data.frame(matrix(NA,nrow=length(years),ncol=length(aboriginal_data$Long_X)))
  for(j in 1:length(aboriginal_data$Long_X)){
    for(i in 1:length(years)){
...
ice_index_bymonthmaxvar=data.frame(matrix(NA,nrow=length(years),ncol=length(aboriginal_data$Long_X)))
for(j in 1:length(aboriginal_data$Long_X)){
  for(i in 1:length(years)){
